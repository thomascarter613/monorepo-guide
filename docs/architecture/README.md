...
Unlike the engineering journal, numbered increment requirements, experiment records, and accepted ADRs, architecture documents are current-state documentation and should be updated when the architecture changes.
...
- `authority-map.md` — provisional concern-by-concern ownership boundaries for mise, Bun, the repository-owned project contract, Moon, Nx, GitHub Actions, the repository command surface, and future ecosystem-native authorities.
- `decision-system.md` — current lifecycle for consequential architecture decisions, experiments, evidence, reconsideration, and supersession.
...
decision-system.md
    How do consequential decisions become accepted architecture,
    how are they evidenced, and how may later evidence replace them?
```
